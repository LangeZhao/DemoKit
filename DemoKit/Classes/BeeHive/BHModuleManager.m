...
static  NSString *kInitAfterCreatedMainScreen = @"modInitAfterCreatedMainScreenEvent:";
static  NSString *kIndexPageLaunchedSelector = @"modIndexPageLaunchedEvent:";
...
static  NSString *kDidRegisterUserNotificationSettingsSelector = @"modDidRegisterUserNotificationSettings:";
...
- (void)triggerEvent:(BHModuleEventType)eventType
{
    switch (eventType) {
        ...
        case BHMInitEvent:
            ...
            break;
        case BHMInitAfterCreatedMainScreenEvent:
            [self handleModuleEvent:kInitAfterCreatedMainScreen];
            break;
        case BHMIndexPageLaunchedEvent:
            [self asyncHandleModuleEvent:kIndexPageLaunchedSelector];
            break;
        ...
        case BHMDidRegisterUserNotificationSettings:
            [self handleModuleEvent:kDidRegisterUserNotificationSettingsSelector];
            break;
        ...
        case BHMHandleWatchKitExtensionRequest:
            [self handleModuleEvent:kHandleWatchKitExtensionRequestSelector];
            break;
...
//        }
        ...
        //支持外部协议设定级别，废弃 basicModuleLevel
        NSUInteger levelInt = 0;
        BOOL responseModuleLevel = [class respondsToSelector:@selector(moduleLevel)];
        if (responseModuleLevel) {
            levelInt = [(Class<BHModuleProtocol>)class moduleLevel];
        }
...
- (void)asyncHandleModuleEvent:(NSString *)selectorStr
{
    SEL seletor = NSSelectorFromString(selectorStr);
    [self.BHModules enumerateObjectsUsingBlock:^(id<BHModuleProtocol> moduleInstance, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([moduleInstance respondsToSelector:seletor]) {
